...
"""Run ALBERT on SQuAD 1.1 and SQuAD 2.0."""
...
class InputFeatures(object):
  ...
  def __init__(self,
    ...
    self.chartok_to_orig_index = chartok_to_orig_index
    self.tokens = tokens
  ...
  for (example_index, example) in enumerate(examples):
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      feature = InputFeatures(
          ...
          chartok_to_orig_index=chartok_to_orig_index,
      ...
      for start_index in start_indexes:
        for end_index in end_indexes:
          ...
          if length > max_answer_length:
            continue
          start_log_prob = result.start_logits[start_index]
          end_log_prob = result.end_logits[end_index]
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    if '-all' == pred_filename[-4:]:
        ...
        golden = '/hdd1/bioalbert/BioASQ/{}_golden.json'.format(task_batch)
    ...
    if os.path.exists(golden):
        ...
        evalproc1 = subprocess.Popen(
            ...
            cwd='/hdd1/bioalbert/Evaluation-Measures',
            stdout=subprocess.PIPE
        )
