...
class QandaProcessor(DataProcessor):
  """processor for the QA task"""
...
def convert_single_ner_example(ex_index, example, label_list, max_seq_length,
                               tokenizer, task_name='ner'):
  """Converts a single `InputExample` into a single `InputFeatures`."""
  ...
  label_ids = []
  tokens_a = []
  for ipos, iword in enumerate(example.text_a.split(" ")):
    previous_len = len(tokens_a)
    tokens_a += [i for i in tokenizer.tokenize(iword)]
    label_ids += [label_map[example.label.split(" ")[ipos].upper()] for _ in range(len(tokens_a) - previous_len)]
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    feature = convert_single_ner_example(ex_index, example, label_list,
